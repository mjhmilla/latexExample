...
%Checks to make sure that there are no obvious errors in the colours
%MM ran this on 2024/05/02 and every colour scheme passed this simple test.
flag_checkColors=0;
if(flag_checkColors==1)
    csFields= fields(cs);
    for i=1:1:length(csFields)
        assert(max(cs.(csFields{i}))<=1);
        assert(max(cs.(csFields{i}))>=0);
    end
end
